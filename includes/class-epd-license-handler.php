...
if ( ! class_exists( 'EPD_License' ) )	{
	...
	class EPD_License {
		...
		public function settings( $settings ) {
            $epd_license_settings = array(
                array(
                    ...
                    'name'    => sprintf( __( '%1$s', 'epd-premium' ), $this->item_name ),
                    'desc'    => '',
                    'type'    => 'license_key',
		...
		public function license_help_text() {
            ...
            printf(
                '<p>' . __( 'Enter your <a href="%s" target="_blank">license keys</a> here to receive updates for extensions you have purchased. If your license key has expired, please renew your license.', 'epd-premium' ) . '</p>',
		...
		public function deactivate_license() {
            ...
            if ( ! wp_verify_nonce( $_REQUEST[ $this->item_shortname . '_license_key-nonce'], $this->item_shortname . '_license_key-nonce' ) ) {
                ...
                wp_die( __( 'Nonce verification failed', 'epd-premium' ), __( 'Error', 'epd-premium' ), array( 'response' => 403 ) );
		...
		public function notices() {
            ...
            if ( is_object( $license ) && 'valid' !== $license->license && empty( $showed_invalid_message ) ) {
                if ( empty( $_GET['tab'] ) || 'licenses' !== $_GET['tab'] ) {
                    $messages[] = sprintf(
                        __( 'You have invalid or expired license keys for Easy Plugin Demo. Please go to the <a href="%s">Licenses page</a> to correct this issue.', 'epd-premium' ),
		...
		public function plugin_row_license_missing( $plugin_data, $version_info ) {
            ...
            if ( ( ! is_object( $license ) || 'valid' !== $license->license ) && empty( $showed_imissing_key_message[ $this->item_shortname ] ) ) {
                echo '&nbsp;<strong><a href="' . esc_url( admin_url( 'edit.php?post_type=epd_ticket&page=epd-settings&tab=licenses' ) ) . '">' . __( 'Enter a valid license key for automatic updates.', 'epd-premium' ) . '</a></strong>';
